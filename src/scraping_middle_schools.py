...
i = 1
middle_url = "https://www.greatschools.org/virginia/schools/?gradeLevels%5B%5D=m&st%5B%5D=public_charter&st%5B%5D=public&view=table&page=" + str(i)

response = requests.get(middle_url)
...
middle_schools_df = pd.DataFrame(
    {'zip_code': zip_code,
...
middle_schools_df['percent_low_income'] = middle_schools_df['percent_low_income'].str.replace('%', '')

middle_schools_df['percent_low_income'] = middle_schools_df['percent_low_income'].replace('?', np.nan)

middle_schools_df.to_csv('../input/middle_schools_data.csv', index=False)
print('exported data: ../input/middle_schools_data.csv')